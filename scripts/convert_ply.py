"""Export a gsplat checkpoint to a viewable .ply file for SuperSplat.

Usage:
    modal run scripts/convert_ply.py --scene driving_test
    modal volume get gaussian-outputs driving_test/driving_test.ply ./driving_test.ply
    # Then drag .ply into https://supersplat.playcanvas.com
"""
import modal
...
def convert(scene: str = "driving_test"):
    import torch
    import numpy as np
    ...
    if sh0.ndim == 3:
        sh0 = sh0[:, 0, :]
    N = len(means)
    print(f"Exporting {N} gaussians from '{scene}'")
...
def main(scene: str = "driving_test"):
    convert.remote(scene=scene)
