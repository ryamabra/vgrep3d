...
The driving scene was captured from a single forward-facing dashcam. With ~292 frames all from roughly the same vantage point, the Gaussian feature field doesn't develop enough per-object separation to localize tight bounding boxes. A scene captured with 360° coverage (walking around objects) would yield significantly better results.

### 2. The feature field operates at scene scale, not instance scale
The current pipeline distills *image-level* SigLIP features into the field. SigLIP's image encoder produces holistic scene embeddings, not per-object segmentation masks. As a result, "red car" activates Gaussians across the whole scene (473k/683k = 69% of Gaussians matched in our test), not just around the actual car.
...
- ✅ Full pipeline: COLMAP → gsplat → feature extraction → autoencoder → field distillation → query
...
If you want tight per-object 3D boxes from a semantic Gaussian field, you likely need:

1. **SAM-guided feature extraction** — use Segment Anything to extract per-object mask features rather than whole-image SigLIP features. Each Gaussian gets features from the masks it belongs to across views.
2. **Instance-aware distillation** — instead of distilling a single feature field, distill a field that separates object instances (similar to SAGA or Gaussian Grouping).
3. **Better capture data** — capture scenes with cameras orbiting objects, not just driving past them.
4. **Spatial clustering as post-processing** — after computing per-Gaussian relevance, apply DBSCAN to find the densest spatial cluster (we tried this — it helps but doesn't fully solve it with dashcam data).
